...
@Service
public class BpmVariableSignUpPersonnelServiceImpl extends ServiceImpl<BpmVariableSignUpPersonnelMapper, BpmVariableSignUpPersonnel> {
    ...
    public void insertSignUpPersonnel(TaskService taskService, String taskId, String processNumber, String nodeId, String assignee, List<String> signUpUsers) {
        ...
        if (bpmVariableSignUp.getAfterSignUpWay() == 1) {//come back to the node who add the sign up node
            ...
            //back
            BpmnConfCommonElementVo backSignUpElement = subElementVos.stream().filter(o -> o.getIsBackSignUp() == 1).findFirst().orElse(new BpmnConfCommonElementVo());
