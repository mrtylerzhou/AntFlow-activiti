<!--
 * 自动节点设置抽屉
 * 复用条件编辑逻辑, 数据源为 auto node 的 conditionList
-->
<template>
            ...
            <el-main>
                <div class="demo-drawer__content">
                    <div class="condition_content drawer_content">
                        <p class="tip">当满足以下条件时，自动节点将执行自定义动作
...
/**保存自动节点设置 */
const saveAutoNode = () => {
    convertConditionNodeValue(originalConfigData.value.conditionList, false)
    ...
    if (updatedNode) {
        ...
        // Build display text
        updatedNode.nodeDisplayName = buildConditionDisplayText(originalConfigData.value.conditionList)
    }
    setAutoNodeConfig({
        value: updatedNode,
        flag: true,
...
/**构建条件显示文本 */
const buildConditionDisplayText = (conditionList) => {
    ...
    return texts.length > 0 ? texts.join(originalConfigData.value.groupRelation ? ' 或 ' : ' 且 ') : ''
}
</script>
