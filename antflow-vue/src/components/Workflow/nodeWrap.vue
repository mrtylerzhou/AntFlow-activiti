            ...
            <div class="title" :style="`background: rgb(${titleBgColor});`">
                ...
                <template v-else>
                    ...
                    <span class="node-badges">
                        <template v-for="(b, idx) in badgeList.list" :key="b.key">
                        ...
                        <svg-icon v-if="badgeList.more" icon-class="more-up" class="node-badge-icon" />
                    </span>
                </template>
            </div>
                        ...
                        <div class="condition-node-box">
                            <div class="node-wrap-box" :class="isTried && item.error ? 'error active' : ''">
                                <div class="title" :style="`background: rgb(${bgColors[4]});`">
                                    ...
                                    <i class="anticon anticon-close close" @click="delTerm(index)"></i>
                                    <span class="node-badges">
                                        <template v-for="(b, idx) in getNodeBadges(item).list" :key="b.key">
                                            <i v-if="b.type === 'anticon'" :class="b.cls"></i>
                                            <span v-else-if="b.type === 'num'" :class="[b.cls, (b.num + '').length > 1 ? 'wide' : '']" :title="b.tip">{{ b.num }}</span>
                                            <svg-icon v-else :icon-class="b.icon" :class="b.cls" />
                                        </template>
                                        <svg-icon v-if="getNodeBadges(item).more" icon-class="more-up" class="node-badge-icon" />
                                    </span>
                                </div>
...
/**
 * 节点角标(node-badges)渲染列表: 判定逻辑统一在 utils/antflow/nodeBadges.js,
 * 竖向与横向设计器共用, 新增角标只需改公共模块的 getBadgeList。
 * 返回 { list: 实际显示角标(>6 截前5), more: 是否显示 more-up }
 */
let badgeList = computed(() => getNodeBadges(props.nodeConfig));
