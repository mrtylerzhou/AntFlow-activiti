...
export let bgColors = [
  ...
  "229, 57, 53",
  "229, 57, 53",
]; // '灰色, 蓝色, 橙色, 黄色, 黄色, , , , 深蓝, 紫色, , , 青绿色(条件审批), 钢蓝色(条件抄送), , , , 靖蓝色(协助), 青色(自动推进), 亮红(自动退回), 亮红(条件退回), 亮红(条件退回发起人)'
...
// 条件推进节点专属颜色(nodeType=12 + condition_advance_node 标签, 满足条件自动推进到固定目标, 与条件审批青绿色区分)
...
/**到达前设置(动态审批人):前端使用的常量。运行期触发为 assignee==-5, 无需节点标签。radio显示值21(避免与根据标签选择20冲突),持久化时映射到 setType=5。*/
export const ARRIVAL_DYNAMIC_SET_TYPE = 21; //radio显示值(映射到 setType=5)
