...
function createContext(options = {}, root = process.cwd()) {
  ...
  const dumpUnimportItems = options.dumpUnimportItems === true ? "./.unimport-items.json" : (_b = options.dumpUnimportItems) != null ? _b : false;
  const resolvers = options.resolvers ? [options.resolvers].flat(2) : [];
  const injectAtEnd = options.injectAtEnd !== false;
  const unimport = createUnimport({
    imports: [],
    presets: (_d = (_c = options.packagePresets) == null ? void 0 : _c.map((p) => typeof p === "string" ? { package: p } : p)) != null ? _d : [],
    injectAtEnd,
